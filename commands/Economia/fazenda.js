...
async function mostrarCanvasFazenda(interaction, fazenda) {
        ...
        if (!buffer || buffer.length === 0) {
                // Se veio do execute (deferReply), usa editReply. Senão, usa reply.
                if (interaction.deferred) {
                        return interaction.editReply("❌ Erro: Buffer de imagem vazio.");
                } else {
                        return interaction.reply("❌ Erro: Buffer de imagem vazio.");
                }
        }
        ...
        const attachment = new AttachmentBuilder(buffer, { name: "fazenda.png" });

        // 🔄 Decide se usa editReply (veio do /fazenda) ou update (veio do menu de seleção)
        let response;
        if (interaction.deferred) {
        ...
        collector.on("collect", async i => {
                ...
                if (i.values[0] === "colher") {
                        ...
                        const sementes = fresca.sementes_plantadas || 1;
                        const producaoPorSemente = 10; // kg por semente
                        const producao = producaoPorSemente * sementes * (fresca.nivel || 1);

                        Database.prepare(`
                                                                                                                        UPDATE fazendas 
                                                                                                                                SET estoque_kg = estoque_kg + ?, ultimo_plantio = NULL, sementes_plantadas = 1
                                                                                                                                        WHERE id = ?
                        ...
                        const embedColheita = new EmbedBuilder()
                                ...
                                .addFields(
                                        ...
                                        { name: "🌱 Sementes", value: `${sementes}x`, inline: true },
                ...
                if (i.values[0] === "voltar") {
                        ...
                        // ✅ CORREÇÃO: usa i.update() pois é uma MessageComponentInteraction
                        return i.update({
                                embeds: [embedSelecao],
                                files: [],
